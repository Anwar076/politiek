<x-admin-layout title="Nieuw evenement">
    <h1 class="text-2xl font-bold mb-8">Nieuw evenement</h1>
    <form action="{{ route('admin.events.store') }}" method="POST" enctype="multipart/form-data" class="max-w-3xl space-y-6">
        @csrf
        <div>
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4"
                      class="w-full px-4 py-3 rounded-lg bg-white/10 border border-white/20 text-white">{{ old('description') }}</textarea>
        </div>
        <div>
